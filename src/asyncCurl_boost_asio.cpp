...
std::map<impl::socket_t, boost::asio::ip::tcp::socket *> socket_map;
                                ...
                                struct curl_sockaddr *address)
{
  ...
  {
    boost::asio::ip::tcp::socket *tcp_socket = new boost::asio::ip::tcp::socket(prio::asioLoop().io());
    ...
    else 
	{
      ...
      socket_map.insert(std::pair<impl::socket_t,boost::asio::ip::tcp::socket *>(sockfd, tcp_socket));
    }
  }
...
int CurlMulti::on_sock_cb(CURL *curl, impl::socket_t sock, int what, CurlEasy* easy)
{
	...
	{
    	std::cout << "\nevent_cb: socket already closed" << std::endl;
    	return 0;
  	}

 	boost::asio::ip::tcp::socket* tcp_socket = socket_map.find(sock)->second;
...
//	curl_easy_setopt(easy_, CURLOPT_VERBOSE, 1L);
	...
	curl_easy_setopt(easy_, CURLOPT_CLOSESOCKETFUNCTION, close_socket);
